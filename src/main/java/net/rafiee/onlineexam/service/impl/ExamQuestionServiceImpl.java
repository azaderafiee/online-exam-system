...
public class ExamQuestionServiceImpl implements ExamQuestionService {
    ...
    public ExamQuestionResponseDTO addQuestionToExam(Long examId, AddQuestionToExamDTO dto, String username) {
        ...
        if (orderNumber == null) {
            // آخرین ترتیب
            orderNumber = examQuestionRepository.findMaxOrderNumberByExamId(examId)
                    .map(max -> max + 1)
                    .orElse(0);
        }
    ...
    public List<ExamQuestionResponseDTO> getExamQuestions(Long examId) {
        ...
        return examQuestionRepository.findByExamIdOrderByOrderNumberAsc(examId).stream()
